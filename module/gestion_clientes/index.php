...
function gc_dispatch($action, &$smarty, $db, $auth, $config, $username, $requestId)
{
    ...
    if ($action === 'api_start_call') {
        ...
        $latestAttempt = $db->fetchOne('SELECT id, technical_state, ended_at, business_outcome_id FROM gc_attempt WHERE agent_map_id=? ORDER BY id DESC LIMIT 1', array($agent['id']));
        ...
        if ($latestAttempt && !empty($latestAttempt['ended_at']) && $latestAttempt['business_outcome_id'] === null) {
    ...
    if ($action === 'api_attempt_status') {
        $agent = $auth->agentMap(false);
        $attempt = $db->fetchOne('SELECT id, phone_id, technical_state, business_outcome_id, originated_at, answered_at, ended_at, duration_seconds, talk_seconds FROM gc_attempt WHERE id=? AND agent_map_id=?', array((int)gc_param('attempt_id', 0), $agent['id']));
    ...
    if ($action === 'workspace') {
        ...
        $campaign = $campaignId ? $db->fetchOne('SELECT id, timezone FROM gc_campaign WHERE id=?', array($campaignId)) : array('timezone' => $config['timezone']);
        if (!$campaign) { $campaign = array('timezone' => $config['timezone']); }
        ...
        $outcomeRequired = $attempt && !empty($attempt['ended_at']) && $attempt['business_outcome_id'] === null;
