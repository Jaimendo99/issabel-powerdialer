...
function gc_dispatch($action, &$smarty, $db, $auth, $config, $username, $requestId)
{
    ...
    if ($action === 'api_attempt_status') {
        $agent = $auth->agentMap(false);
        $attempt = $db->fetchOne('SELECT id, technical_state, business_outcome_id, originated_at, answered_at, ended_at, duration_seconds, talk_seconds FROM gc_attempt WHERE id=? AND agent_map_id=?', array((int)gc_param('attempt_id', 0), $agent['id']));
