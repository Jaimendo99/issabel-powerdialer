...
class GestionClientesImport
{
    ...
    {
        $parsed = $this->parse($path, $mapping, $limit);
        return array(
            'delimiter' => $parsed['delimiter'],
            'total' => $parsed['total'],
            'accepted' => count($parsed['accepted']),
            'rejected' => count($parsed['errors']),
            'duplicates' => $parsed['duplicates'],
            'sample' => array_slice($parsed['accepted'], 0, 20),
    ...
    public function commit($campaignId, $path, $originalFilename, $mapping, $actor, $ip)
    {
        ...
        return $db->transaction(function ($tx) use ($campaignId, $originalFilename, $mapping, $actor, $ip, $parsed, $hash) {
            ...
            return array('batch_id' => (int)$batchId, 'accepted' => count($parsed['accepted']), 'rejected' => count($parsed['errors']), 'duplicates' => $parsed['duplicates']);
        });
    }

    private function parse($path, $mapping, $limit)
    {
        $delimiter = $this->detectDelimiter($path);
        $handle = fopen($path, 'rb');
        if (!$handle) {
        ...
        fclose($handle);
        return array('delimiter' => $delimiter, 'total' => $rowNumber - 1, 'accepted' => $accepted, 'errors' => $errors, 'duplicates' => $duplicates);
    }
